...
无需重启，即可发现soterkey检测过了，原理是就是降级攻击+服务屏蔽
```
原本：
...
- 一加（OxygenOS/ColorOS）在**Bootloader解锁**后，硬件TEE的**attestation证书链**会被系统/谷歌/腾讯的**在线吊销列表（revocation list）**标记为**已吊销/不可信**。  
  → Network 部分明确显示：“This certificate chain matched 1 revoked/suspended entry.”  
  →  Soter check 就会出现 `finalErr=4`、`signing=skipped`、`removeAskSkipped=false` 等错误。
- 春秋 Native check-Eros 3.1 和 DuckDetector **最近更新**专门加强了 **SoterKey 检测**，不再只是简单看 root/hook，还会主动调用 SoterService 做完整性校验 → 直接红字“System compromised” + “TEE 环境不可信”。
...
| **冻结 SoterService**<br>(com.tencent.soter.soterserver) | 服务正常运行 → 检测通过 | **直接冻结** | 让检测工具**无法连接 Treble 服务** → 强制走**降级路径** |

- **“降级攻击”** 的本质就是：**让 Soter 服务不可达** → 检测逻辑直接**跳过**所有严格检查：
...
-  **Soter check skipped because the Treble service was not reachable**，正是冻结生效后的标志。
- “Normal” + “未发现高风险”，就是因为所有 Soter 步骤都变成了“跳过”，新版检测把这种状态判定为**安全**（设计上为了兼容老设备）。
